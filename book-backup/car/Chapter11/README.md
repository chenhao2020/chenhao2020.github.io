...
我总是使用命令`-o /home/pi/gnss/rtkrover.conf`来加载配置文件`rtkrover.conf`。文件位于`/home/pi/gnss/rtkrover.conf`。使用–o参数，将在RTKRCV程序的启动过程中自动加载所有配置。
...
RTKRCV服务器正常工作，必须配置配置文件`rtkrover.conf`。我将文件存在`/home/pi/gnss/rtkrover.conf`中。
